...
storms %>%
  group_by(name) %>%
  summarize(max_category = max(category), max_wind_speed = max(wind), max_pressure = max(pressure), max_ts_diameter = max(ts_diameter, na.rm = TRUE), max_hu_diameter = max(hu_diameter, na.rm = TRUE))
```
...
storm_count_per_year = storms %>%
  ...
  tally() %>%
  plot()
# no pattern. nothing worth noting
...
storms %>%
  group_by(status) %>%
  summarize(average_lat = mean(lat), average_long = mean(long))
```
...
storms %>%
  group_by(status) %>%
  summarize(average_hours = mean(hour), median_hours = median(hour))
```
...
storms %>%
  unite(timestamp, year, month, day, hour)
...
storms %>%
  group_by(category) %>%
  summarize(average_wind_speed =mean(wind), average_pressure = mean(pressure), average_ts_diameter = mean(ts_diameter, na.rm = TRUE), average_hu_diameter = mean(hu_diameter, na.rm = TRUE))
```
...
ggplot(Boston, aes(x = rm, y = medv)) +
  geom_point(aes(col = tax, size = crim))
...
```{r}
interaction_model = lm(medv ~ rm * tax * crim, Boston)
regular_model_boston = lm(medv ~ rm + tax + crim, Boston)
summary(interaction_model)$r.squared
summary(interaction_model)$sigma
summary(regular_model_boston)$r.squared
summary(regular_model_boston)$sigma
```
...
```{r}
ggplot(Boston, aes(x = rm, y = medv)) +
  geom_point(aes(col = age, size = lstat))
```
...
```{r}
interaction_model2 = lm(medv ~ rm * age * lstat, Boston)
regular_model_boston2 = lm(medv ~ rm + age + lstat, Boston)
summary(interaction_model2)$r.squared
summary(interaction_model2)$sigma
summary(regular_model_boston2)$r.squared
summary(regular_model_boston2)$sigma
```
