...
# directions unclear if one line is meant towards row and one line towards column or one line for both using apply function once. 
```
...
# could have used sum. But this works as well. trying with different code to learn more. 
```{r}
...
# got help on this problem from classmate. was stuck on pct_missing and first_NA
...
# yes the estimate is about what is expected by theory. As n increases these values would converge.
```{r}
quantile(v, probs = (.05))
...
# ecdf is base R function that finds CDF. taking the value at 0 gives the percentile we were looking for. multplying by 100 was not needed. Used it to look cleaner. 
```{r}
f = ecdf(v)
f(0) * 100
#this is about right with theory, 85, increase n and these values would converge.
```
...
#while this function ONLY provides a rough indication of the memory being used, it does make sense for the higher dimension arrays using up more memory. The more dimesions you have, the more space being taken up, in essence, the more memory being used. 
...
lorem_mixed = sample(unlist(strsplit(lorem, split = " ")))
paste(lorem_mixed, collapse = " ")
