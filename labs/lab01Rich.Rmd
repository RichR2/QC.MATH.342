...
options(decimal = 10)
pi
# options digits did not give ten digits after decimal. corrected in TA office hours
```
...
No because of floating point error. discussed in class
...
NaN come from log(-number). Inf comes from log(Inf) and -Inf comes from log(0).
...
factor(x)
```
yes because 53 unique elements in the vector.
...
* Use `x` to create a new vector `y` containing only real numbers in x.
...
*** this one was saved for lab 2
``` {r}
# this problem was saved for lab 2
``` 

* Use the `strsplit` function and `sample` to put the sentences below in random order.
*** this problem was also moved to lab 2 by teacher.
```{r}
...
#repeating message here, professor told class not to do this problem
```
...
x3 = factor(sample(x_3, 100, replace = TRUE), levels = x_3, exclude = NA, ordered = FALSE, nmax = NA)
x3
# exclude = NA and nmax = NA not needed, added in to follow basic syntax shown in ?factor. This was done to simply get used to syntax and basics of functions. 
```

* Use `x_3` to create `x_3_bin`, a binary feature where 0 is no crime and 1 is any crime.

```{r}
x_3_bin = as.numeric(x3 != "none")
x_3_bin 
#updated code. previous code was with ifelse. produced correct code, but did not store variables correctly. this one works better, approved by professor in lab.
```

* Use `x_3` to create `x_3_ord`, an ordered, nominal factor variable. Ensure the proper ordinal ordering.

```{r}
x_3_ord = ordered(x_3 , levels = x_3, exclude = NA)
x_3_ord
# factor or ordered will work. Used ordered to get familiar with different functions. Professor saw and approved.
```
