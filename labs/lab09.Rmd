...
```{r}
in_sample_brier_by_iteration
oos_brier_by_iteration
oos_brier


#oos worse then in sample. about what we would expect. model is not terrible though. Does not look like we overfit. With how much data we have and the not too many variables (relatively) we did not overfit.
```
...
#THE SIGNS MAKE SENSE FOR THE MOST PART. MORE EDUCATION ELUDES TO A GREATER CHANCE OF MAKING > 50K, similary being a farmer in 1994 most likely did not, leading to negative signs.
...
head(predict(logistic_mod, Xmm_train, type = "response"))
head(predict(logistic_mod, Xmm_test,type = "response"))
head(predict(logistic_mod, Xmm_select, type = "response"))
```
...
```{r}
#practice lecture
p_hats_test = predict(logistic_mod, Xmm_test, type = "response")
pacman::p_load(ggplot2)
ggplot(data.frame(p_hats_test = p_hats_test, ytrain = ytrain)) + 
  geom_histogram(aes(x = p_hats_test, fill = factor(ytrain)))
#no. not good at all. 
```
...
```{r}
#practice lecture
y_hats_train = factor(ifelse(phatTrain >= 0.5, ">50K", "<=50K"))
y_hats_test = factor(ifelse(p_hats_test >= 0.5, ">50K", "<=50K"))
mean(y_hats_test != ytest)
oos_conf_table = table(ytest, y_hats_test)
oos_conf_table
...
```{r}
## this is from practice lecture. not mine.
...
performance_metrics_oos_sample = compute_metrics_prob_classifier(phat_select, y_hats_test) %>% data.table
performance_metrics_oos_sample
```
...
```{r}
pacman::p_load(ggplot2)
ggplot(performance_metrics_oos_sample) +
...
-trapz(performance_metrics_oos_sample$FPR, performance_metrics_oos_sample$recall)
#worse then the purely random model. Something went wrong.
```
...
performance_metrics_oos_sample$avg_cost = 
  (c_fp * performance_metrics_oos_sample$FP +
  c_fn * performance_metrics_oos_sample$FN )
ggplot(performance_metrics_oos_sample) +
...
```{r}
#tried to base off practice lecture. something went wrong along the way. led to a chain reaction.
optimal_model = which.max(performance_metrics_oos_sample$avg_cost)
performance_metrics_oos_sample[optimal_model, ] 
```
